...
def axis_angle(R):
    ...
    if theta < 1e-10:
        ...
        omega = np.multiply(theta / (2 * np.sin(theta)), [R[2, 1] - R[1, 2], R[0, 2] - R[2, 0], R[1, 0] - R[0, 1]])
    return omega
...
def computelie(lie_params):
    ...
    for j in range(njoints):
        if j == 0:
            A[j, :, :] = lietomatrix(lie_params[j + 1, 0:3].T, lie_params[j, 3:6].T)
        elif j == njoints - 1:
            A[j, :, :] = np.matmul(np.squeeze(A[j - 1, :, :]),
                                   lietomatrix(np.array([0, 0, 0]).T, lie_params[j, 3:6].T))
        else:
            A[j, :, :] = np.matmul(np.squeeze(A[j - 1, :, :]),
                                   lietomatrix(lie_params[j + 1, 0:3].T, lie_params[j, 3:6].T))
...
def rotmat(A):
    ...
    if theta == 0:
        ...
        A = A / theta
        cross_matrix = np.array([[0, -A[2], A[1]], [A[2], 0, -A[0]], [-A[1], A[0], 0]])
        R = np.eye(3) + np.sin(theta) * cross_matrix + (1 - np.cos(theta)) * np.dot(cross_matrix, cross_matrix)
    return R
